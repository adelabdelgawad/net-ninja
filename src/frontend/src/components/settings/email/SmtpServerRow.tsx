  ...
  return (
    <div class="group flex items-center gap-3 px-4 py-3 hover:bg-accent transition-colors rounded-[8px]">
      ...
      <div
        ...
        classList={{
          'bg-[#10b981]/15': props.config.isDefault && props.config.isActive,
          'bg-primary/15': !props.config.isDefault && props.config.isActive,
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          <span class="text-[13px] font-medium text-foreground truncate">{props.config.name}</span>
          <span class="px-2 py-0.5 text-[10px] rounded-[3px] bg-accent text-muted-foreground border border-border">
            {VENDOR_CONFIGS[props.config.vendor].displayName}
          </span>
          <Show when={props.config.isDefault}>
            <span class="px-1.5 py-0 text-[10px] leading-4 font-medium rounded-full bg-[#10b981]/20 text-[#57e389]">
              Default
            </span>
          </Show>
          <Show when={!props.config.isActive}>
            <span class="px-1.5 py-0 text-[10px] leading-4 font-medium rounded-full bg-[#808080]/15 text-muted-foreground">
              Inactive
            </span>
          </Show>
        </div>
