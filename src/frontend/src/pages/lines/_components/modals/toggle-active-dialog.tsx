...
export interface ToggleActiveDialogProps {
  open: boolean;
  line: Line | null;
  toggling: boolean;
...
export const ToggleActiveDialog: Component<ToggleActiveDialogProps> = (props) => {
  const willActivate = () => !(props.line?.isActive ?? true);

  return (
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent class="max-w-[400px]">
        <DialogHeader>
          <DialogTitle>{willActivate() ? 'Activate' : 'Deactivate'} Line</DialogTitle>
          <button
            type="button"
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-[13px] text-[#cccccc] mb-1">
                ...
                <strong class="text-white">"{props.line?.name}"</strong>?
              </p>
              <p class="text-[11px] text-[#808080]">
                {willActivate()
                  ? 'This line will be included in task executions and monitoring.'
                  : 'This line will be excluded from task executions and monitoring.'}
              </p>
            </div>
          </div>
        </div>
